...
export default function EditVideoDialog({ video, onClose, onSuccess }) {
  const [videoId, setVideoId] = useState(video?.videoId || "defaultVideoId");
  const [firstVideoId, setFirstVideoId] = useState(
    video?.firstVideoId || "defaultFirstVideoId"
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/playlists/${video.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ videoId, firstVideoId, title, createdAt }),
      });
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            ...
            <label className="text-white font-medium">
              Youtube ID of First Video of Playlist
              <input
                type="text"
                value={firstVideoId}
                onChange={(e) => setFirstVideoId(e.target.value)}
                className="w-full mt-1 p-2 rounded bg-[#222831] text-white border border-[#00ADB5] focus:outline-none"
              />
            </label>
